...
"""
    ...
    for i in range(1, len(versions) + 1):
        ...
        if args.UseCustomNanoAOD and step == 'step7': # FIXME: This is a temporary fix
            script_content += f"pushd ${{{step}}}/src\n"
            script_content += f"eval `scram runtime -sh`\n"
            script_content += f"git cms-addpkg PhysicsTools/NanoAOD\n"
            script_content += f"git cms-addpkg RecoTauTag/RecoTau\n"
            script_content += f"git clone https://github.com/cms-data/RecoTauTag-TrainingFiles.git RecoTauTag/TrainingFiles/data\n"
            script_content += f"pushd RecoTauTag/TrainingFiles/data/DeepTauId\n"
            script_content += f"wget https://github.com/gparida/HeavyMassResonanceHH/raw/main/DeepBoostedTrainingModels/LatestInUse/deepTau_2018v2p7_core.pb\n"
            script_content += f"wget https://github.com/gparida/HeavyMassResonanceHH/raw/main/DeepBoostedTrainingModels/LatestInUse/deepTau_2018v2p7_inner.pb\n"
            script_content += f"wget https://github.com/gparida/HeavyMassResonanceHH/raw/main/DeepBoostedTrainingModels/LatestInUse/deepTau_2018v2p7_outer.pb\n"
            script_content += f"wget https://github.com/gparida/HeavyMassResonanceHH/raw/main/DeepBoostedTrainingModels/LatestInUse/full_model_June8_postBUGfx.pb\n"
            script_content += f"popd\n"
            script_content += f"git remote add ganesh https://github.com/gparida/cmssw\n"
            script_content += f"git fetch ganesh\n"
            script_content += f"git checkout -b renanoHHbbtt_13_0_13  ganesh/renanoHHbbtt_13_0_13\n"
            script_content += f"mkdir bbtautauAnalysisScripts\n"
            script_content += f"pushd bbtautauAnalysisScripts\n"
            script_content += f"git init\n"
            script_content += f"git remote add -f origin https://github.com/gparida/bbtautauAnalysisScripts.git\n"
            script_content += f"git config core.sparseCheckout true\n"
            script_content += f"git sparse-checkout init\n"
            script_content += f"git sparse-checkout set boostedTauLeadingLeptonIso TauLeadingLeptonIso utilities CRABSubmission\n"
            script_content += f"git pull origin master\n"
            script_content += f"popd\n"
